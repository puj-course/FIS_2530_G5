...
public class LoginController {
            ...
            switch (resultado) {
                case 0:
                    ...
                    if (usuarioId > 0) {
                        ...
                        boolean verificacion_Admin = UsuarioService.VerificarAdmin(usuarioId);
                        handleGoToProfile(usuarioId, correo,verificacion_Admin);
     ...
     */
    private void handleGoToProfile(int usuarioId, String correo , boolean admin ) {

        if(admin){
            try {
                FXMLLoader loader = new FXMLLoader(getClass().getResource("AdminView.fxml"));
                Parent root = loader.load();
                Vista_Admin_BloqueosController controller = loader.getController();
    ...
    private void handleGoToSignup() {
        try {
            ...
            Stage modalStage = new Stage(); // Nuevo Stage
            modalStage.setScene(new Scene(root, 500, 700));
            modalStage.setTitle("GREENET - Registro");
            modalStage.centerOnScreen();
            modalStage.initModality(Modality.APPLICATION_MODAL);
            modalStage.initOwner(((Stage) correoField.getScene().getWindow())); // 🔗 Ventana principal como dueño
            modalStage.showAndWait(); // Espera a que se cierre antes de continuar
        } catch (Exception e) {
